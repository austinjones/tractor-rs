...
impl TractorStorage {
    pub fn new(name: &str) -> Result<TractorStorage, ResourceError> {
        let mut path = dirs::home_dir().unwrap();
        path.push("Tractor");
        path.push("storage-".to_string() + name);

        TractorStorage::from_path(path)
    }

    pub(crate) fn from_path(storage_loc: PathBuf) -> Result<TractorStorage, ResourceError> {
        if storage_loc.is_file() {
            return Err(ResourceError::TractorStorageIsFile);
        }
    ...
    pub fn stream<'a>(&'a self, name: &str) -> TractorStream<'a> {
        TractorStream::new(&self, name)
    }
    ...
    pub(crate) fn contains(&self, key: Uuid, format: Format) -> bool {
    ...
    pub(crate) fn path(&self, uuid: Uuid, format: Format) -> PathBuf {
        let mut path = self.storage_loc.clone();
    ...
    fn index_loc(path: &Path) -> PathBuf {
        ...
        buf.push(".tractor-index.txt");
        buf
    }
}

pub struct TractorStream<'a> {
    storage: &'a TractorStorage,
    name: String,
}
...
impl<'a> TractorStream<'a> {
    pub(crate) fn new(storage: &'a TractorStorage, name: &str) -> TractorStream<'a> {
        TractorStream {
            storage,
            name: name.to_string(),
        }
    ...
    fn index_loc(&self) -> PathBuf {
        let filename = ".tractor-stream-".to_string() + self.name.as_str() + ".txt";
...
impl<'a> IntoIterator for &'a TractorStream<'a> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        let items = match self.new_items() {
            Ok(v) => v.into_iter(),
            Err(e) => {
                println!("Tractor stream error: {}", e);
                Vec::new().into_iter()
            }
        };
